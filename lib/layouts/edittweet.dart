...
class EditTweet extends StatefulWidget {
  final id, tweet, docid;
  const EditTweet({Key key, this.id, this.docid, this.tweet}) : super(key: key);

  @override
  _EditTweetState createState() => _EditTweetState();
}

class _EditTweetState extends State<EditTweet> with SingleTickerProviderStateMixin {
  ...
  update() async {
    ...
    var doc2 = widget.id;
    await FirebaseFirestore.instance.collection('timeline').doc('$doc1').update({'time': _currenttime, 'tweet': tweet.text, 'profile': profile});
    await FirebaseFirestore.instance
        .collection('$email')
        .doc('$doc2')
        .update({'time': _currenttime, 'tweet': tweet.text, 'profile': profile})
        .then((value) => Navigator.pop(context))
        .then((_) {
          Navigator.of(context).pop();
          Fluttertoast.showToast(msg: 'tweet updated', toastLength: Toast.LENGTH_SHORT, gravity: ToastGravity.BOTTOM);
        });
  }
